...
//window.addEventListener?window.addEventListener("load",createGradient(document.body),false):window.attachEvent("onload",createGradient(document.body));

function createGradient(element) {
	if(!document.getElementById)return;
	
	objArray = $(element).select('[class~="gradient"]');
	...
	for(i=0;i<objArray.length;i++) {
		...
		if(params[3] == "horizontal" && objArray[i].offsetWidth == 0) {
		  continue;
		} else if (params[3] == "vertical" && objArray[i].offsetHeight == 0) {
		  continue;
		}
		...
		if(document.all && !window.opera) {
			...
			if(params[3] == "horizontal") {
			  colorArray = createColorPath(params[1],params[2], objArray[i].offsetWidth);
				w=Math.round(objArray[i].offsetWidth/colorArray.length);
				if(!w)w=1;
				h = objArray[i].offsetHeight;
			} else {
			  colorArray = createColorPath(params[1],params[2], objArray[i].offsetHeight);
			...
			g = document.createElement("div");
			params[3] == "horizontal"?w=objArray[i].offsetWidth-(colorArray.length*w)+1:h=objArray[i].offsetHeight-(colorArray.length*h);
			g.setAttribute("style","position:absolute;z-index:0;top:" + y + "px;left:" + x + "px;height:" + h + "px;width:" + w + "px;background-color:rgb(" + colorArray[p-1][0] + "," + colorArray[p-1][1] + "," + colorArray[p-1][2] + ");");
			tmpDOM.appendChild(g);
...
/*
function getGradientObjects(element) {
